...
We include the option of Algebraic Subgrid Scale (ASGS) stabilization for the velocity and density models.  The ASGS term is in the form of a strong residual $R$ tested against the linear adjoint of our operator $\mathcal{L}$ on a test function. 
\begin{equation}
...
$\Gamma/\Omega$. Note that the characteristic function of the set
...
%% distribution at the ``downstream'' boundary.
...
\end{equation}
Due the level set and volume fraction correction, we know furthemore that $\rho_1 \leq \rho \leq \rho_0$.
...
\item [Step 4] Pressure Correction: Solve for $\psi^{\sharp}$ precisely as above with $\chi=\rho_1$
