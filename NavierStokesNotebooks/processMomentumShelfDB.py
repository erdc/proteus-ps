...
numTimeSteps = {}
vel_maxL2Norm = vel_maxH1Norm = velpp_maxL2Norm = p_maxL2Norm = {}
vel_ell2L2Norm = vel_ell2H1Norm = velpp_ell2L2Norm = p_ell2L2Norm = {}

for i in range(num_filenames):
    numTimeSteps[i] = len(database[i]['timeValues']) 
    ...
    print "Maximum in time norms for database %1d with numTimeSteps = %05d:" %(i,numTimeSteps[i])
...
if num_filenames > 1:
    ...
    rate_vel_maxL2Norm = rate_vel_maxH1Norm = rate_velpp_maxL2Norm = rate_p_maxL2Norm = [0]*num_filenames
    rate_vel_ell2L2Norm = rate_vel_ell2H1Norm = rate_velpp_ell2L2Norm = rate_p_ell2L2Norm = [0]*num_filenames
    for i in range(1,num_filenames):
        rate_vel_maxL2Norm[i] = np.log(vel_maxL2Norm[i]/vel_maxL2Norm[i-1])/np.log(numTimeSteps[i]/numTimeSteps[i-1])
        rate_vel_maxH1Norm[i] = np.log(vel_maxH1Norm[i]/vel_maxH1Norm[i-1])/np.log(numTimeSteps[i]/numTimeSteps[i-1])
        rate_velpp_maxL2Norm[i] = np.log(velpp_maxL2Norm[i]/velpp_maxL2Norm[i-1])/np.log(numTimeSteps[i]/numTimeSteps[i-1])
        rate_p_maxL2Norm[i] = np.log(p_maxL2Norm[i]/p_maxL2Norm[i-1])/np.log(numTimeSteps[i]/numTimeSteps[i-1])
    
        rate_vel_ell2L2Norm[i] = np.log(vel_ell2L2Norm[i]/vel_ell2L2Norm[i-1])/np.log(numTimeSteps[i]/numTimeSteps[i-1])
        rate_vel_ell2H1Norm[i] = np.log(vel_ell2H1Norm[i]/vel_ell2H1Norm[i-1])/np.log(numTimeSteps[i]/numTimeSteps[i-1])
        rate_velpp_ell2L2Norm[i] = np.log(velpp_ell2L2Norm[i]/velpp_ell2L2Norm[i-1])/np.log(numTimeSteps[i]/numTimeSteps[i-1])
        rate_p_ell2L2Norm[i] = np.log(p_ell2L2Norm[i]/p_ell2L2Norm[i-1])/np.log(numTimeSteps[i]/numTimeSteps[i-1])
    
    
    
    print "numTS\tvel_maxL2  rate    vel_maxH1  rate    vpp_maxL2  rate    pre_maxL2  rate"
    for i in range(num_filenames):
