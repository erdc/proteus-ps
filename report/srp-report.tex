...
\begin{equation}
  ...
  \end{split}
\end{equation}
Notice that we only use $\phi_h^{k+1}$ or $\rho_h^{*}$ in the above equation.
...
\begin{align}
  ...
  &= \left[\rho_h\frac{d\ub_h}{dt} \right]^{k+1} + \frac{1}{2} \left[ \frac{d\rho_h}{dt} +\nabla\cdot\left(\rho_h\ub_h\right)\right]^{k+1}\ub_h^{k+1} + \mathcal{O}(\tau^2)\\
  &=\left[\rho_h\frac{d\ub_h}{dt} \right]^{k+1} + \mathcal{O}(\tau^{2})
\end{align}
where we have used the fact that $\ub_h^{*}$ is a second order in time approximation to $\ub_h^{k+1}$ and the conservation of mass property of continuous equations.
\end{remark}
