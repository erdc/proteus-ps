...
\subsection{CFL Condition For Variable Step BDF1 Algorithm}
...
%
%
\section{Summary of BDF2 Algorithm with Comparison in context of BDF1 algorithm}

\subsection{Time discretizations}
Let $D_{\beta} g^{k+1} = \beta_0g^{k+1} - \displaystyle\sum_{i=1}^{m} \beta_i g^{k+1-i} \approx \frac{\partial g}{\partial t}(t^{k+1})$ be the m-th order discretization of the first derivative operator.  For example, with uniform time steps we have
\begin{equation}
 D_{\beta} g^{k+1} = \begin{cases}
...
%
%  
\subsection{General BDF1-BDF2 algorithm}
