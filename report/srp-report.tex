...
% \end{equation}
% Everything else is kept the same as above.  The convergence results have not been proved for the case of variable time stepping but it is reasonable to suspect that under proper CFL conditions with a maximal time step enforced as well
%
% \begin{equation}
%   \tau^{k+1} \leq \min {c h_{min}, \tau^{max}}, $c<1$
% \end{equation}
